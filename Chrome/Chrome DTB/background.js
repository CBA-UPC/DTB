...
chrome.storage.sync.get(['allowed_urls'], function(result){
    if(Object.keys(result).length != 0){
        ...
        //console.log("URLs recovered from memory: ", user_allowed_urls);
    }
});
...
chrome.storage.sync.get(['allowed_hosts'], function(result){
    if(Object.keys(result).length != 0){
        ...
        //console.log("Hosts recovered from memory: ", user_allowed_hosts);
    }
});
...
function processResult(prepro_url){
    ...
    //console.log("After reshape", result.dataSync());
...
chrome.extension.onMessage.addListener(function(request, sender, sendResponse) {
    ...
    case 'reload_tab':
        ...
        chrome.tabs.executeScript(current_tab, {code: code});
        break;
	}
